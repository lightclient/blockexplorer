...
public class HuntAndKillMazeAlgorithm : MazeAlgorithm {
	...
	public HuntAndKillMazeAlgorithm(MazeCell[,] mazeCells, int height) : base(mazeCells) {
		generator = new ProceduralNumberGenerator (height);
	}
	...
	private void Kill() {
		while (RouteStillAvailable (currentRow, currentColumn)) {
			...
			int direction = generator.GetNextNumber ();

			if (direction == 1 && CellIsAvailable (currentRow - 1, currentColumn)) {
				// North
				DestroyWallIfItExists (mazeCells [currentRow, currentColumn].north);
				DestroyWallIfItExists (mazeCells [currentRow - 1, currentColumn].south);
				currentRow--;
			} else if (direction == 2 && CellIsAvailable (currentRow + 1, currentColumn)) {
				// South
				DestroyWallIfItExists (mazeCells [currentRow, currentColumn].south);
				DestroyWallIfItExists (mazeCells [currentRow + 1, currentColumn].north);
				currentRow++;
			} else if (direction == 3 && CellIsAvailable (currentRow, currentColumn + 1)) {
				// east
				DestroyWallIfItExists (mazeCells [currentRow, currentColumn].east);
				DestroyWallIfItExists (mazeCells [currentRow, currentColumn + 1].west);
				currentColumn++;
			} else if (direction == 4 && CellIsAvailable (currentRow, currentColumn - 1)) {
				// west
				DestroyWallIfItExists (mazeCells [currentRow, currentColumn].west);
				DestroyWallIfItExists (mazeCells [currentRow, currentColumn - 1].east);
				currentColumn--;
			}
	...
	private void DestroyAdjacentWall(int row, int column) {
		...
		while (!wallDestroyed) {
			...
			if (direction == 1 && row > 0 && mazeCells [row - 1, column].visited) {
				DestroyWallIfItExists (mazeCells [row, column].north);
				DestroyWallIfItExists (mazeCells [row - 1, column].south);
				wallDestroyed = true;
			} else if (direction == 2 && row < (mazeRows-2) && mazeCells [row + 1, column].visited) {
				DestroyWallIfItExists (mazeCells [row, column].south);
				DestroyWallIfItExists (mazeCells [row + 1, column].north);
				wallDestroyed = true;
			} else if (direction == 3 && column > 0 && mazeCells [row, column-1].visited) {
				DestroyWallIfItExists (mazeCells [row, column].west);
				DestroyWallIfItExists (mazeCells [row, column-1].east);
				wallDestroyed = true;
			} else if (direction == 4 && column < (mazeColumns-2) && mazeCells [row, column+1].visited) {
				DestroyWallIfItExists (mazeCells [row, column].east);
				DestroyWallIfItExists (mazeCells [row, column+1].west);
				wallDestroyed = true;
			}
		}
